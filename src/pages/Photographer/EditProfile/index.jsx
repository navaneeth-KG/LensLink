...
const PgEditProfile = () => {
  ...
  const onChange2 = (e, key) => {
   ...
   const onEdit=async()=>{
   const response= await axios.patch(`http://localhost:4999/photographer/${id}`,pg)
        ...
        {services.map(item => {
          return (
            ...
            <input
                ...
                name={item.name}
                id={item.name}
                onChange={(e)=>{onChange2(e,'service')}}
                
              />
              <div className="label"><label htmlFor={item.name} >{item.name}</label></div>
              
              </div>
              <input type='number' placeholder='enter price'onChange={(e)=>{onChange2(e,'price')}} style={{height:'30px'}}/>
              <button onClick={onClick} style={{color:'red',width:'50px',height:'30px'}}>add</button>
            </div>
